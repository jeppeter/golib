...
func Aescbcenc_handler(ns *extargsparse.NameSpaceEx, ostruct interface{}, ctx interface{}) (err error) {
	var sarr []string
	var key []byte
	var iv []byte
	var data []byte
...
func Aescbcdec_handler(ns *extargsparse.NameSpaceEx, ostruct interface{}, ctx interface{}) (err error) {
	var sarr []string
	var key []byte
	var iv []byte
	var data []byte
	...
	plaintext, err = aesext.AesDecCbc(data,key,iv)
	if err != nil {
		return
	}

	_, err = fileop.WriteFileBytes(ofile,plaintext)
	if err != nil {
		return
	}
...
func init() {
	Aescbcenc_handler(nil, nil, nil)
	Aescbcdec_handler(nil, nil, nil)
}
func main() {
	...
	commandline = `{
		"input|i" : null,
		"output|o" : null,
		"aescbcenc<Aescbcenc_handler>##aeskey ivfile datafile with output to output option##" : {
			"$" : 3
		},
		"aescbcdec<Aescbcdec_handler>##aeskey ivfile datafile with output to output option##" : {
			"$" : 3
		}
